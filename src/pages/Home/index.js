...
const Page = () => {
  const { data } = useData();
   ...
   // fair la conparaison des objet du tableaux pour trouver l'événement le plus récent
 const last = data && data.events && data.events.length > 0
  ? data.events.reduce((mostRecent, currentEvent) => {
      const mostRecentDate = new Date(mostRecent.date);
      const currentEventDate = new Date(currentEvent.date);
      return currentEventDate > mostRecentDate ? currentEvent : mostRecent;
    })
  : null;
  return (
    <>
